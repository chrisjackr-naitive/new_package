...
from xraysynthesis.utils.imagemanipulation import (
    draw_straight_lines,
    link_curves,
...
class LesserTrochanterXRaySynthesis(xraysynthesis.XRaySynthesis):
    ...
    def generate_region_of_interest_and_labels(self, roi_magnitude):
        ...
        bone_image, a_points = beziercurve.draw_bezier_curves(
            bone_image,
            points["inner_cortical_a"],
            color=1,
            thickness=1,
            curvatures=[(0, 0.5)],
            return_all_points=True,
        )
        bone_image, c_points = beziercurve.draw_bezier_curves(
            bone_image,
            points["inner_cortical_c"],
            color=m * roi * cbone,
            thickness=2,
            curvatures=[(0, 0.5)],
            return_all_points=True,
        ...
        half_points = link_curves(
            b_points + [np.asarray([520, -5])], [np.asarray([640 + 10, 640 + 10])]
        )
        ...
        bone_image = fill_poly_with_texture(
            bone_image,
            np.int32(inner_cortical_points),
        ...
        bone_image = fill_poly_with_texture(
            bone_image,
            np.int32(outter_cortical_points),
            np.ones(bone_image.shape),
            max_color=m * roi * cbone,
            min_color=roi * cbone,
        )
    ...
    def create_image_with_xray_noise_and_label(self):
        ...
        if self.place_random_text:
            for i in range(np.random.randint(7)):
                imagemanipulation.add_text_to_image(
                    region_of_interest_image, roi_magnitude
                )
