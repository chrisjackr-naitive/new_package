...
BONE_POINT = {
        ...
        'c3': np.array([[640+10, 128]]),
        'c1': np.array([[480, 640+10]]),
         }
...
def add_linear_artefacts(image):
    ...
    if num>6:
        # DARK ARTEFACT
        s = randint(20,150)
        e1 = np.asarray([-5, randint(75,260)])
        e3 = np.asarray([260, e1[1] - randint(0,30) - s])
    ...
    if num>8:
        #LIGHT FATFOLD
        s = randint(20,150)
        e1 = np.asarray([-5, randint(75,230)])
...
class LesserTrochanterXRaySynthesis(xraysynthesis.XRaySynthesis):
    ...
    def generate_region_of_interest_and_labels(self, roi_magnitude):
        ...
        c1 = scaled_points['c1']  + [[s,0]]    + [[c1_x,0]]
        c3 = scaled_points['c3']  + [[s,0]]    + [[0,c3_y]]
        c2 = np.mean((c1,c3),axis=0)

        points = {}
        points['inner_cortical_a'] = np.concatenate((a1,a2,a3), axis=0)
        points['inner_cortical_c'] = np.concatenate((c1,c2,c3), axis=0)

        # ==================== GENERATE ROI IMAGE =====================
        bone_image = np.ones((image_size, image_size), dtype=np.float32)*uniform(0,0.05)

        # CORTICAL LINES
        bone_image, a_points = beziercurve.draw_bezier_curves(bone_image, points['inner_cortical_a'], color=1, thickness=1, curvatures=[(0,0.5)],return_all_points=True)
        bone_image, c_points = beziercurve.draw_bezier_curves(bone_image, points['inner_cortical_c'], color=m*roi*cbone, thickness=2, curvatures=[(0,0.5)], return_all_points=True) 
        ...
        points['inner_cortical_b'] = b_array

        c_array = np.asarray(c_points)
        d_array = (c_array + [randint(10,40),0]).tolist()   
        d_points = [np.asarray(x) for x in d_array]
        points['inner_cortical_d'] = d_array
        ...
        whole_points = link_curves(a_points+[np.asarray([520, -5])], d_points)
        half_points = link_curves(b_points+[np.asarray([520, -5])], [np.asarray([640+10, 640+10])])
        inner_cortical_points = link_curves(a_array, b_array)
        outter_cortical_points = link_curves(c_array, d_array)
        ...
        bone_image = fill_poly_with_texture(bone_image, np.int32(outter_cortical_points), np.ones(bone_image.shape), max_color=m*roi*cbone, min_color=roi*cbone)
        ...
        if not hidden:
            ...
            troch_points += [np.asarray([640,0])]#curve
            ...
            bone_image = combine_with_overlay(bone_image, overlay,  uniform(0.4,1))
        elif hidden:
